    ...
    for (uint i = 1; i < plsize; i++) {
      if ((msg->points[i].line < N_SCANS) &&
        ...
        if (valid_num % point_filter_num == 0) {
          //          if(i==1){
          //              std::printf("Scan timestamp: %f, First point time:
          ...
          //与上一个添加进去的点比较
          if (((abs(pl_full[i].x) > 1e-7) || (abs(pl_full[i].y) > 1e-7) ||
               (abs(pl_full[i].z) > 1e-7)) &&
...
void Preprocess::velodyne_handler(
  ...
  if (feature_enabled) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      // 还有可能当前帧的点云中出现了不属于这一帧的点
      // 即时间戳超出当前帧采样范围太远 这样的点也要丢弃
      // 否则可能会影响pcl_end_time的计算 这个会给imu
      // propagation的过程的时间戳产生误导
      // 这个问题可能是雷达传输过程中的丢失重传造成的 也可能是雷达内部的bug
      if (std::abs(pl_orig.points[i].time) > (1.0 / SCAN_RATE) * 1.1) {
        //            std::printf("PT timestamp out of range: %d    frame ts: %f
        //            point ts: %f\n",
        //                       i, msg->header.stamp.toSec(),
...
void Preprocess::rs_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  ...
  //  FIXME:  时间戳大于0.1
  auto time_list_robosense = [&](robosense_ros::Point &point_1,
                                 robosense_ros::Point &point_2) {
    return (point_1.timestamp < point_2.timestamp);
  };
  sort(pl_orig.points.begin(), pl_orig.points.end(), time_list_robosense);
  while (pl_orig.points[plsize - 1].timestamp - pl_orig.points[0].timestamp >=
         0.1) {
    plsize--;
  ...
  double timespan_ =
      pl_orig.points.back().timestamp - pl_orig.points[0].timestamp;
  ...
  //           "
  //           << msg->header.stamp.toSec() - pl_orig.points.back().timestamp <<
  //           std::endl;
  for (int i = 0; i < pl_orig.points.size(); i++) {
